...
$no=$_REQUEST['no'];
$sql="select name, hit, subject, text from php_board where no='$no'";
...
?>
<a href="board_modify.php?no=<?=$no?>">수정</a> <a href="board_delete.php?no=<?=$no?>">삭제</a>
<?
} 
?>
</td>
...
$hit="update php_board set hit=hit+1 where no='$no'";
...
<table id="cn">
<?
$sqlc="select * from php_cmt where no='$no'";
...
<!--코멘트 쓰기-->
<? if ($_SESSION['idx']){?>
<form action="board_comment.php?no=<?=$no?>" method="post" name="cmm">
...
<table>
<tr>
<td align="right"><a href="board_list.php">목록</a> <a onclick="prev()">이전</a> <a onclick="next()">다음</a></td>
</tr>
</table>
...
function prev(){ //이전으로 연결 /// no-1이 0이라면 반환하지 않음.
	...
	if (contirm){
	location.href='board_view.php?no=2';
	}
	}
	else {
	location.href="board_view.php?no=<?=$no-1?>";	
	}
}
