...
$sql="select no, subject, id, date, hit from board_drama";
...
$cn=$row['no'];
$cnt="select cmtno from cmt_drama where no='$cn'";
...
<?   } ?>
<td id="gr"><?=$row['no']?></td>
<td align="left"><a href="dramaboard_view.php?no=<?=$row['no']?>"><?=$row['subject']?></a> <span id="gr">[<?=$count?>]</span></td>
<td><?=$row['id']?></td>
...
if ($_SESSION['idx']){
?><a href="dramaboard_write.php?id=<?=$_SESSION['idx']?>">글쓰기</a>
<?
	} } else {
?>
