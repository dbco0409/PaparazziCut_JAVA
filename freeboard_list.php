...
$sql="select no, subject, name, date, hit from board_free";
...
$cn=$row['no'];
$cnt="select cmtno from cmt_free where no='$cn'";
...
<?   } ?>
<td id="gr"><?=$row['no']?></td>
<td align="left"><a href="freeboard_view.php?no=<?=$row['no']?>"><?=$row['subject']?></a> <span id="gr">[<?=$count?>]</span></td>
<td><?=$row['name']?></td>
...
if ($_SESSION['idx']){
?><a href="freeboard_write.php?id=<?=$_SESSION['idx']?>">글쓰기</a>
<?
	} } else {
?>
