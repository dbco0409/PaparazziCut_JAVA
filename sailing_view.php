...
?>
<a href="sailing_2modify.php?no=<?=$no?>">수정</a> <a href="delete.php?no=<?=$no?>&board=sailing_2">삭제</a>
<?
} 
?>
</td>
...
<input type="text" value="" />(<?=$opt1?>)<? } ?>
    ...
    <tr>
        <td width="310" colspan="2">총상품가격</td>
    </tr>
    <tr>
        <td width="310" colspan="2"><input type="button" value="구매하기" /> &nbsp;<input type="button" value="장바구니" /></td>
    </tr>
</table>
<table border="1" width="636">
    <tr>
...
function prev(){ //이전으로 연결 /// no-1이 0이라면 반환하지 않음.
	...
	if (contirm){
	location.href='sailing1_view.php?no=2';
	}
	}
	else {
