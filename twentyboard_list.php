...
$sql="select no, subject, name, date, hit from board_twenty";
...
$cn=$row['no'];
$cnt="select cmtno from cmt_twenty where no='$cn'";
...
<?   } ?>
<td id="gr"><?=$row['no']?></td>
<td align="left"><a href="twentyboard_view.php?no=<?=$row['no']?>"><?=$row['subject']?></a> <span id="gr">[<?=$count?>]</span></td>
<td><?=$row['name']?></td>
...
if ($_SESSION['idx']){
?><a href="twentyboard_write.php?id=<?=$_SESSION['idx']?>">글쓰기</a>
<?
	} } else {
?>
