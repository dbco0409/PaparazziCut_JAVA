...
$no=$_REQUEST['no'];
$sql="select name, hit, subject, text from board_twenty where no='$no'";
...
?>
<a href="twentyboard_modify.php?no=<?=$no?>">수정</a> <a href="delete.php?no=<?=$no?>&board=twenty">삭제</a>
<?
} 
?>
</td>
...
$hit="update board_twenty set hit=hit+1 where no='$no'";
...
<table id="cn">
<?
$sqlc="select * from cmt_twenty where no='$no'";
...
<!--코멘트 쓰기-->
<? if ($_SESSION['idx']){?>
<form action="twentyboard_comment.php?no=<?=$no?>" method="post" name="cmm">
...
<table>
<tr>
<td align="right"><a href="twentyboard_list.php">목록</a> <a onclick="prev()">이전</a> <a onclick="next()">다음</a></td>
</tr>
</table>
...
function prev(){ //이전으로 연결 /// no-1이 0이라면 반환하지 않음.
	...
	if (contirm){
	location.href='twentyboard_view.php?no=2';
	}
	}
	else {
	location.href="twentyboard_view.php?no=<?=$no-1?>";	
	}
}
